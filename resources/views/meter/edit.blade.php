        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                  <form action="{{ url('/meters/edit/update/'.$meter->metercodeid ) }}" method="POST" id="form">
                      @csrf
                      <div class="mb-6">
                          ...
                          <select class="w-full px-3 py-2 placeholder-gray-300 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-100 focus:border-indigo-300 dark:bg-gray-700 dark:text-white dark:placeholder-gray-500 dark:border-gray-600 dark:focus:ring-gray-900 dark:focus:border-gray-500" name="client_id">
                            ...
                            @foreach($clients as $key => $client)
                              <option value="{{$client->id}}" @if(old('client_id')==$client->id) selected @elseif($meter->client_id==$client->id) selected @endif>{{$client->names}}</option>
                            @endforeach
                          </select>
                          @error('client_id')
                      ...
                      <div class="mb-6">
                          ...
                          <input type="text" name="meterlandupi" id="meterlandupi" placeholder="Enter Land upi" class="w-full px-3 py-2 placeholder-gray-300 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-100 focus:border-indigo-300 dark:bg-gray-700 dark:text-white dark:placeholder-gray-500 dark:border-gray-600 dark:focus:ring-gray-900 dark:focus:border-gray-500"  value="{{old('meterlandupi')!=""?old('meterlandupi'):$meter->meterlandupi}}"/>
                          @error('meterlandupi')
                            <div class="text-red-600 ...">{{ $message }}</div>
                          @enderror
                      </div>
                      <div class="mb-6">
                          ...
                          <input type="text" name="pipeline" id="pipeline" placeholder="Enter pipe line" class="w-full px-3 py-2 placeholder-gray-300 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-100 focus:border-indigo-300 dark:bg-gray-700 dark:text-white dark:placeholder-gray-500 dark:border-gray-600 dark:focus:ring-gray-900 dark:focus:border-gray-500"  value="{{old('pipeline')!=""?old('pipeline'):$meter->pipeline}}"/>
                          @error('pipeline')
                            <div class="text-red-600 ...">{{ $message }}</div>
                          @enderror
                      </div>
